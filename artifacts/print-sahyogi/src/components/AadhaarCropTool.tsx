...
const FRONT_CROP = { x: 101.404, y: 1149.72, w: 497.067, h: 313.6 } as const;
const BACK_CROP  = { x: 626.773, y: 1149.72, w: 497.067, h: 313.6 } as const;
...
function SliderRow({
  ...
  return (
    <div className="flex items-center gap-3">
      <span className="w-20 text-xs font-medium text-gray-600 shrink-0">{label}</span>
      <input
        type="range"
        min={min}
        max={max}
        ...
        className={`flex-1 h-1.5 rounded-full cursor-pointer ${accentClass}`}
      />
      <span className={`w-9 text-right text-xs font-mono font-semibold tabular-nums shrink-0 ${
        value > 0 ? 'text-blue-600' : value < 0 ? 'text-orange-600' : 'text-gray-400'
      }`}>
        {value > 0 ? `+${value}` : value}
      </span>
    </div>
...
export function AadhaarCropTool() {
  ...
  // CSS filter for real-time profile photo preview
  const profileCssFilter = useMemo(() => {
  ...
  // Extract profile photo whenever frontImage changes
  useEffect(() => {
    if (!frontImage) return;
    const img = new Image();
    img.onload = () => {
      const { x, y, w, h } = PROFILE_REGION;
      const px = Math.floor(x * img.width);
      const py = Math.floor(y * img.height);
      const pw = Math.floor(w * img.width);
      ...
      if (ctx) {
        ...
        setProfilePhotoUrl(canvas.toDataURL('image/png'));
      }
    };
    img.src = frontImage;
  }, [frontImage]);

  // ── Apply adjustments to front card ──────────────────────────────────────────
  const handleApplyAdjustments = useCallback(async () => {
    ...
    // Apply adjustments ONLY to the profile photo region
    const { x, y, w, h } = PROFILE_REGION;
    const px = Math.floor(x * img.width);
    const py = Math.floor(y * img.height);
    const pw = Math.floor(w * img.width);
    ...
    setAdjustedFrontUrl(newFrontUrl);
    setAdjustmentsApplied(true);

    // Regenerate A4 with the adjusted front card
    const imgs: string[] = [newFrontUrl];
    if (backImage) imgs.push(backImage);
    await generateA4Preview(imgs);

    setIsApplying(false);
  }, [frontImage, backImage, brightness, contrast, exposure]);
  ...
  const renderPageForCrop = async (page: pdfjsLib.PDFPageProxy) => {
  ...
  const extractSinglePageCards = async (page: pdfjsLib.PDFPageProxy) => {
    const [frontCanvas, backCanvas] = await Promise.all([
      renderCropRegion(page, FRONT_CROP),
      renderCropRegion(page, BACK_CROP),
    ]);
    return [frontCanvas.toDataURL('image/png'), backCanvas.toDataURL('image/png')];
  };
  ...
  const processPDFPages = async (pdf: pdfjsLib.PDFDocumentProxy) => {
    try {
      ...
      if (pdf.numPages === 1) {
        const page = await pdf.getPage(1);
        images.push(...(await extractSinglePageCards(page)));
      } else {
  ...
  const effectiveFront = adjustedFrontUrl ?? frontImage;
  const handleDownload      = () => { if (previewDataUrl)   triggerDownload(previewDataUrl,   `${baseName}_A4.png`); };
  const handleDownloadFront = () => { if (effectiveFront)   triggerDownload(effectiveFront,   `${baseName}_Front.png`); };
  ...
  const renderStep2 = () => (
    <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
      ...
      {error && <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="text-red-500 text-sm font-medium">{error}</motion.div>}
    ...
    <motion.div key="step3" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }}
      ...
      <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 w-full max-w-2xl text-sm text-amber-800">
        ...
        <span>
          ...
          <strong className="text-blue-600">Back</strong> (left side). Confirm to crop both.
        </span>
      </div>
      <div className="w-full max-w-2xl bg-gray-100 rounded-2xl p-3 md:p-6 shadow-inner">
        <div ref={previewWrapperRef} className="relative w-full overflow-hidden rounded-lg shadow">
          {pagePreviewDataUrl && (
            <>
              <img src={pagePreviewDataUrl} alt="Aadhaar PDF preview" className="w-full block" onLoad={updateOverlay} />
              {cropBoxes && (
                <>
  ...
  const renderStep5 = () => (
    <motion.div key="step5" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div className="flex flex-col w-full max-w-4xl mb-6 gap-3 print:hidden">
        ...
        <div className="flex gap-2">
          <button onClick={handleDownloadFront} disabled={!effectiveFront}
      ...
      {/* ── Profile Photo Adjustments Panel ─────────────────────────────────── */}
      {profilePhotoUrl && (
        <div className="w-full max-w-4xl mb-6 print:hidden">
          <div className="bg-gray-950 rounded-2xl p-5 border border-gray-800">
            ...
            <div className="flex items-center gap-2 mb-5">
              ...
              <span className="text-sm font-semibold text-gray-200">Profile Photo — Adjustments</span>
              <span className="ml-auto text-xs text-gray-500 italic">Only affects the photo on the front card</span>
            </div>

            <div className="flex flex-col sm:flex-row gap-6 items-center sm:items-start">
              {/* Photo preview */}
              <div className="flex flex-col items-center gap-2 shrink-0">
                <div className="w-24 h-28 rounded-xl overflow-hidden border-2 border-gray-700 shadow-lg bg-gray-900">
                  <img
                    src={profilePhotoUrl}
                    alt="Profile"
                    style={{ filter: profileCssFilter, width: '100%', height: '100%', objectFit: 'cover' }}
                    className="transition-all duration-200"
                  />
                </div>
                <span className="text-[10px] text-gray-500 uppercase tracking-wide">Live Preview</span>
              </div>
              ...
              <div className="flex-1 w-full space-y-5">
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button
                    ...
                    disabled={isApplying}
                    className="flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold transition disabled:opacity-60"
                  >
                    {isApplying
                      ? <><Loader2 className="h-3.5 w-3.5 animate-spin" />Applying…</>
                      : <><Sparkles className="h-3.5 w-3.5" />Apply to Output</>
                    }
                  </button>
                </div>
                ...
                {adjustmentsApplied && (
                  <p className="text-xs text-green-400 flex items-center gap-1.5">
                    ...
                    Adjustments baked into Front card and A4 download.
                  </p>
                )}
              </div>
            </div>
      ...
      <div className="w-full max-w-2xl bg-gray-100 p-4 md:p-8 rounded-2xl print:p-0 print:bg-white print:max-w-none">
        <div id="print-layout"
          ...
          {effectiveFront && (
            <img src={effectiveFront} alt="Aadhaar Front" className="w-[85%] object-contain mt-12 mb-6" />
          )}
          {backImage && (
